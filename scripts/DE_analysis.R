...
mean.expression[which(mean.expression$experimental - mean.expression$control > fc.threshold),"gene_type"] <- "activado"
mean.expression[which(mean.expression$experimental - mean.expression$control < -(fc.threshold)),"gene_type"] <- "reprimido"
...
log.fold.change <- de.results$log2FoldChange
q.values <- de.results$padj

activated.genes <- row.names(de.results)[log.fold.change > fc.threshold & q.values < q.val.threshold]
activated.genes <- activated.genes[!is.na(activated.genes)]

repressed.genes <- row.names(de.results)[log.fold.change < - fc.threshold & q.values < q.val.threshold]
...
volcano_DEG<-print(ggplot(as.data.frame(de.results), aes(x=log2FoldChange, y=-log10(padj),
                                                   color=gene_type,text= paste0("</br> Gen: " ,gene_name))) +
  geom_point(size = 1) +
  scale_colour_manual(values = volcol) + 
  theme(legend.position = "none",
...
volcano_DEG<-ggplotly(volcano_DEG,x = ~log2FoldChange, y = ~padj,
                  tooltip = "text",
                  width = 450, height = 500)
...
write(x = paste(c("In this section we present the results from a differential gene expression 
                   analysis performed using a **q-value of ", q.val.threshold, "** and 
                   a **fold-change of ", fc.threshold, "**. The volcano
                   plots below represents the gene differential expression. In this
                  graph the red and blue dots represent activated and repressed
                  genes respectively. The grey dots represent genes that not 
                  differentially expressed. The x-coordinate
                  represents the **log2 gene expression fold-change** and the 
                  y-coordinate represent the **minus log10 q-value**. Similarly,
                  grey, red and blue dots represent non differentially expressed,
                  activated and repressed genes respectively.\n 
